...
void idTarget_CallObjectFunction::Event_Activate( idEntity* activator )
{
	...
	for( i = 0; i < targets.Num(); i++ )
	{
		...
		if( ent != NULL && ent->scriptObject.HasObject() )
		{
			...
			if( func == NULL )
			{
				gameLocal.Warning("Function '%s' not found on entity '%s' for function call from '%s'", funcName, ent->name.c_str(), name.c_str());
				return;
			}
			if( func->type->NumParameters() != 1 )
			{
				gameLocal.Warning("Function '%s' on entity '%s' has the wrong number of parameters for function call from '%s'", funcName, ent->name.c_str(), name.c_str());
			}
			if( !ent->scriptObject.GetTypeDef()->Inherits( func->type->GetParmType( 0 ) ) )
			{
				gameLocal.Warning("Function '%s' on entity '%s' is the wrong type for function call from '%s'", funcName, ent->name.c_str(), name.c_str());
			}
